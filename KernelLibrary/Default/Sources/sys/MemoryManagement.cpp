...
static Memory::Management MemoryManager[4];
...
unsigned long Memory::malloc(unsigned long Size) {
    ...
    if(MemoryManager[0].Available == true) {
        return MemoryManager[0].malloc(Size);
    }
    else {
        return MemoryManager[1].malloc(Size);
    }
}

void Memory::free(void *Address) {
    if((((unsigned long)Address) >= MemoryManager[0].StartAddress) && (((unsigned long)Address) < MemoryManager[0].EndAddress)) {
        MemoryManager[0].free(Address);
        return;
    }
    if((((unsigned long)Address) >= MemoryManager[1].StartAddress) && (((unsigned long)Address) < MemoryManager[1].EndAddress)) {
        MemoryManager[1].free(Address);
        return;
    }
}